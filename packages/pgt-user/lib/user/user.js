PGT.namespacer('PGT.User', {user :
	function user(serverThis) {
		var user = Meteor.user()
		if (user) return new PGT.User.model(user);
		else return null;
	}
});

PGT.namespacer('PGT.User', {ID :
	function ID() {
		return Meteor.userId();
	}
});

PGT.namespacer('PGT.User', {findOne :
	...
	function findOne(selector, sort) {
		...
		return Meteor.users.findOne(selectObj, { 
			...
			transform : function(doc) {
				return new PGT.User.model(doc);
			}
		});
	}
});

PGT.namespacer('PGT.User', {find :
...
// instance methods
_.extend(PGT.User.model.prototype, {
